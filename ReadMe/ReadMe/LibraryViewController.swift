...
class LibraryDataSource: UITableViewDiffableDataSource<Section, Book> {
    ...
    func update(sortStyle: SortStyle, animatingDifferences: Bool = true) {
      ...
      for (readMe, books) in booksByReadMe {
        ...
        switch sortStyle {
        case .title:
            sortedBooks = books.sorted(by: { $0.title.localizedCaseInsensitiveCompare($1.title) == .orderedAscending })
        case .author:
            sortedBooks = books.sorted(by: { $0.author.localizedCaseInsensitiveCompare($1.author) == .orderedAscending })
        case .readMe:
            sortedBooks = books
        }
